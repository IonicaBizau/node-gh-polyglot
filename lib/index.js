...
GhPolyglot.prototype.getAllRepos = function (page, repos, callback) {
    ...
    repos = repos || [];
    return JsonRequest({
        url: "https://api.github.com/users/" + self.user + "/repos?per_page=100&page=" + page
      , headers: {
            "User-Agent": "GitHub Polyglot"
        }
    }, function (err, newRepos, res) {
...
GhPolyglot.prototype.repoStats = function (callback) {
    return JsonRequest({
        url: "https://api.github.com/repos/" + this.user + "/" + this.repo + "/languages"
      , headers: {
            "User-Agent": "GitHub Polyglot"
        }
    }, this.check(callback));
